...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    if (!guildSettings){
      ...
      return interaction.reply('Le bot a mit à jour la base de données pour votre serveur, retapez la commande.');
    }
    if (interaction.isCommand() || interaction.isContextMenu()) {
      ...
      if (cmd.ownerOnly){
        if(interaction.user.id != ownwerId) return interaction.reply('La seule personne pouvant taper cette commande est le propriétaire du bot.')
      }
